...
export async function GET(request: Request) {
  try {
    ...
    const { result, response } = await callApi(`/sites/faq/data?${queryString}`);
    ...
    return NextResponse.json({
      ...
      message: 'faq tidak tersedia saat ini',
      error: error.message || 'Gagal mengambil data faq'
    }, { status: 200 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const { result, response } = await callApi('/sites/faq/add', {
      method: 'POST',

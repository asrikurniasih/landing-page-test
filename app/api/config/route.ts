...
export async function GET(request: Request) {
  try {
    ...
    const { result, response } = await callApi(`/sites/config/data?${queryString}`);
    ...
    return NextResponse.json({
      ...
      message: 'Config tidak tersedia saat ini',
...
export async function POST(request: Request) {
  try {
    ...
    const { result, response } = await callApi('/sites/config/add', {
      method: 'POST',

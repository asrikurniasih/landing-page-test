...
export async function GET(request: Request) {
  try {
    ...
    const { result, response } = await callApi(`/sites/faq_category/data?${queryString}`);
    ...
    return NextResponse.json({
      ...
      message: 'Kategori faq tidak tersedia saat ini',
      error: error.message || 'Gagal mengambil data kategori faq'
    }, { status: 200 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const { result, response } = await callApi('/sites/faq_category/add', {
      method: 'POST',
